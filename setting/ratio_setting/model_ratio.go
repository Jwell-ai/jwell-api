...
// All pricing defaults are intentionally empty — ratios, prices and completion
// ratios are synced from the upstream pricing API on every bootstrap.
...
func GetCompletionRatio(name string) float64 {
	name = FormatMatchingModelName(name)
	// Map (upstream-synced) takes priority over hardcoded defaults.
...
func GetCompletionRatioInfo(name string) CompletionRatioInfo {
	name = FormatMatchingModelName(name)
	// Map (upstream-synced) always wins; hardcoded is the fallback, never locked.
	if ratio, ok := completionRatioMap.Get(name); ok {
